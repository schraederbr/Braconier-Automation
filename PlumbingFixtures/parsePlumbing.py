...
class plumbingJob:
    class partRow:
        def __init__(self, id, description: str, quantity, size_strength = None, manufacturer = "unknown", row = None):
            self.row = row
            self.id = id
...
def parseFixtures(sourcePath, sourceQuantityCol, priceCol, sourceStartRow):
    ...
    for row in iterator:
        ...
        if currentCell is not None and currentCell != 0:
            ...
            price = sourceSheet[priceCol + currentRow].value
            #finalSheet[priceCol + str(i)].value = price
            thisJob.addRow(plumbingJob.partRow(id=tagID, quantity=quantity, size_strength=size, description=description, manufacturer=manufacturer))
            i += 1
    return thisJob
...
if __name__ == '__main__':
    ...
    for source in sourcePaths:
        myJobs.append(parseFixtures(source, sourceQuantityCol, priceCol, sourceStartRow))
    rowCounter = 2
    rowIndex = {}
    ...
    finalSheet['A1'] = "Identification"
    finalSheet['B2'] = "Size/Strength"
    for i, j in enumerate(myJobs):

        j.column = chr(ord('A') + (i + len(sourcePaths)))
        finalSheet[chr(ord(j.column)+i*2) + str(1)] = j.title
        finalSheet[chr(ord(j.column)+i*2) + str(2)] = 'Manufacturer'
        finalSheet[chr(ord(j.column)+i*2+1) + str(2)] = 'Qnantity'
        for row in j.rows:
            ...
            for r in rowIndex:
                if row.equals(r):
                    ...
                    finalSheet[chr(ord(j.column)+i*2) + str(rowIndex[r])] = row.manufacturer
                    if("ERROR" in str(row.quantity)):
                        finalSheet[chr(ord(j.column)+i*2+1) + str(rowIndex[r])].fill = my_fill
                    finalSheet[chr(ord(j.column)+i*2+1) + str(rowIndex[r])] = row.quantity
                    break
            else:
                row.row = rowCounter
                ...
                finalSheet[chr(ord(j.column)+i*2) + str(row.row)] = row.manufacturer
                if("ERROR" in str(row.quantity)):
                    finalSheet[chr(ord(j.column)+i*2+1) + str(row.row)].fill = my_fill
                finalSheet[chr(ord(j.column)+i*2+1) + str(row.row)] = row.quantity
